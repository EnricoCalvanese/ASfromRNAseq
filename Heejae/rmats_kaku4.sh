...
ALIGNED_DIR=/global/scratch/users/enricocalvane/heejae_as/kaku_data/aligned_bams
OUTPUT_BASE=/global/scratch/users/enricocalvane/heejae_as/kaku_data/rmats_output
TMP_DIR=/global/scratch/users/enricocalvane/heejae_as/kaku_data/rmats_tmp

# Create output directories
mkdir -p $OUTPUT_BASE $TMP_DIR

# Create BAM list files
# For wild type (col) samples
echo "${ALIGNED_DIR}/SRR27228279_col_2.unique.sorted.bam,${ALIGNED_DIR}/SRR27228280_col_1.unique.sorted.bam,${ALIGNED_DIR}/SRR27228278_col_3.unique.sorted.bam" > $OUTPUT_BASE/wt_bams.txt

# For kaku4 samples
echo "${ALIGNED_DIR}/SRR27228276_kaku4_2.unique.sorted.bam,${ALIGNED_DIR}/SRR27228277_kaku4_1.unique.sorted.bam,${ALIGNED_DIR}/SRR27228275_kaku4_3.unique.sorted.bam" > $OUTPUT_BASE/kaku4_bams.txt
...
echo "Running kaku4 vs WT comparison..."
run_rmats "kaku4_vs_wt" \
    $OUTPUT_BASE/wt_bams.txt \
    $OUTPUT_BASE/kaku4_bams.txt
